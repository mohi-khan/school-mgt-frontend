...
import { useGetBankPaymentReport, useGetAllStudents } from '@/hooks/use-api'
...
const BankPaymentReport = () => {
  ...
  const { data: paymentReports } = useGetBankPaymentReport(fromDate, toDate)
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Bank Payment Report')
    ...
    saveAs(blob, `bank-payment-report-${fromDate}-to-${toDate}.xlsx`)
  }

  const generatePdf = async () => {
    const targetRef = document.getElementById('bank-payment-report-content')
    if (!targetRef) return
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.setFontSize(10)
      const baseText = `Bank Payment Report from ${fromDate} to ${toDate} ( Date : `
      pdf.setFont('bold')
    ...
    pdf.save(`bank-payment-report-${fromDate}-to-${toDate}.pdf`)
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Bank Payment Report</h2>
      ...
      {/* Report Content */}
      <div id="bank-payment-report-content" className="space-y-6">
        {!fromDate || !toDate ? (
                  ...
                  <TableBody>
                    {filteredPayments.map((report, index) => (
                      <TableRow key={report.studentPaymentId || index}>
                        ...
                        <TableCell>{report.bankName || 'N/A'}</TableCell>
                        <TableCell>{report.accountNumber || 'N/A'}</TableCell>
                        <TableCell>{report.branch || 'N/A'}</TableCell>
