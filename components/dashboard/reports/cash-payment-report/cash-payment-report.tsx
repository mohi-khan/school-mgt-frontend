...
import { useGetCashPaymentReport, useGetAllStudents } from '@/hooks/use-api'
import { formatDate, formatNumber } from '@/utils/conversions'
import { CustomCombobox } from '@/utils/custom-combobox'

const CashPaymentReport = () => {
  const [fromDate, setFromDate] = useState('')
  const [toDate, setToDate] = useState('')
  const [selectedStudentId, setSelectedStudentId] = useState<string>('')

  const { data: students } = useGetAllStudents()
  const { data: paymentReports } = useGetCashPaymentReport(fromDate, toDate)
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Cash Payment Report')
    ...
    saveAs(blob, `cash-payment-report-${fromDate}-to-${toDate}.xlsx`)
  }

  const generatePdf = async () => {
    const targetRef = document.getElementById('cash-payment-report-content')
    if (!targetRef) return
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pdf.setFontSize(10)
      const baseText = `Cash Payment Report from ${fromDate} to ${toDate} ( Date : `
      pdf.setFont('bold')
    ...
    pdf.save(`cash-payment-report-${fromDate}-to-${toDate}.pdf`)
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold">Cash Payment Report</h2>
      ...
      {/* Report Content */}
      <div id="cash-payment-report-content" className="space-y-6">
        {!fromDate || !toDate ? (
          ...
          <Card className="shadow-md">
            <CardContent className="p-8 text-center text-gray-500">
              No cash payment records found for the selected criteria
            </CardContent>
          </Card>
        ) : (
          <Card className="shadow-md">
